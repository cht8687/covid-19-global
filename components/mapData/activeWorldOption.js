...
export const activeWorldOption = (name, data, total, timestamp) => ({
  title: [
    {
      textStyle: {
      ...
      text: upperCase(name) + ' Active cases map ',
      top: 'auto',
      subtextStyle: {
        color: colours.dimWhite,
        fontSize: 12,
  ...
  geo: {
    name: 'Coronavirus COVID-19 Confirmed Cases',
    type: 'map',
    map: 'world',
    zoom: '1.5',
    roam: 'scale',
  ...
  series: [
    {
      ...
      data: data
        .map(function(itemOpt) {
          if (!itemOpt.code) return;
          if (!latlong[itemOpt.code]) return;
          return {
            name: itemOpt.name,
            value: [
